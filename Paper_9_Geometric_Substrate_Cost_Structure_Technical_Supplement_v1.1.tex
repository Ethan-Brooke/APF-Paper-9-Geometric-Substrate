...
\hypersetup{
  ...
  pdftitle={Paper 9 Technical Supplement - Dependency-Closed Conditional Package}
}
...
\title{Paper 9 Technical Supplement\\
\large Dependency-Closed Conditional Package for Response-Induced Geometry of the Admissibility Substrate}
\author{E.\ S.\ Brooke \\[0.4em] \normalsize ORCID: \href{https://orcid.org/0009-0001-2261-4682}{0009-0001-2261-4682}}
\date{Version 1.1 --- July 2026}
...
\begin{abstract}
This Technical Supplement is a dependency-closed conditional proof package for Paper~9, \emph{The Geometric Substrate as Cost Structure of Comparison Continuations}.  It reproduces the premises used by the certified native chain and states every standard mathematical import with its exact hypotheses.  The finite operational-basis theorem is proved in conditional general form and machine-certified on the finite atom-cover instantiation; a separate physical realization gate is required before an operational basis becomes a precision-channel basis.  The record topology, differential-space construction, subcartesian locus, continuation-orbit geometry, derivative-kernel identity, and response-orbit quotient gauge are each stated with their named channel, countability, smooth-generation, flow, and first-order-faithfulness gates.  A physical comparison-substrate anchor, quadraticity, Lorentzian signature, the weak-field no-trace source law, and the nonlinear Paper~6 endpoint remain open, conditional, or external.  No external APF reading is required for the certified native core; Paper~6 remains the separately marked source of record for the represented nonlinear endpoint.
\end{abstract}
...
\section{Closure certificate and provenance ledger}\label{sec:ts-closure}

\begin{center}
\begin{tabularx}{\textwidth}{@{}lX@{}}
\toprule
Package & Paper 9 Technical Supplement, Version 1.1\\
Canonical main paper & Paper 9 --- \emph{The Geometric Substrate as Cost Structure of Comparison Continuations}\\
Upstream material reproduced & Paper 1 distinction/identity closure; Paper 10 continuation calculus and the conditional finite operational-basis package; Paper 14 held continuation profile; Paper 37 held-to-record typing reproduced as a declared structural reading\\
Standard imports stated & Initial-topology separation; Sikorski differential spaces; subcartesian realization; complete-family orbit theorem; constant-rank quotient theorem\\
Certified native scope & Conditional construction from jointly realizable operational classes to record topology, differential space, regular continuation orbits, derivative-faithful response quotient, and intrinsic response-orbit gauge\\
Required external APF reading & None for the certified native core; Paper~6 is required only to audit the separately marked represented nonlinear GR endpoint\\
Named conditional/open gates & Finite joint extension and compatible joint minimum realization in the general physical domain; operational-basis--to--precision-channel realization; countable topology generation; finite smooth generation; complete local continuation flows; smooth response-factor completeness; physical substrate anchor; ledger calibration; quadraticity or Finsler branch; causal signature; response Hessian/source; no-trace; nonlinear GR translation\\
Engine status & APF-Engine v24.3.427 contains the exact finite atom-cover basis triad and the split Interface Engine exports/obstructions; publication freeze additionally requires a clean full-bank run, reviewed atlas re-pin, and regenerated reviewer artifacts\\
Certificate status & Dependency-closed conditional package; physical-realization closure remains explicitly open\\
\bottomrule
\end{tabularx}
\end{center}

\paragraph{Provenance rule.}
A theorem may have a canonical home elsewhere in the APF corpus while being reproduced here in full.  Canonical ownership controls maintenance; local reproduction controls reviewer self-containment.  The local statements below govern this supplement.

\section{Closed upstream operational foundation}\label{sec:ts-foundation}

\begin{definition}[Enforceable distinction]\label{def:ts-enforceable-distinction}
An enforceable distinction on a physical interface $U$ is a tested alternative whose two outcomes can be maintained, propagated through an admissible continuation, and deposited in a physical record with nonzero operational consequence.  Two descriptions that differ only by data with no possible continuation or record witness represent the same physical distinction class.
\end{definition}

\begin{definition}[Closed finite interface and marginal floor]\label{def:ts-closed-interface}
A closed interface $U$ has finite available enforcement capacity $C_U<\infty$.  There is a positive marginal distinction floor $\floor>0$: every genuinely additional tested distinction committed jointly with an existing realization increases minimum committed capacity by at least $\floor$.
\end{definition}

\begin{principle}[Contextual continuation identity]\label{pr:ts-contextual-identity}
Physical identity is equality across every admissible future continuation context.  Consequently, a coordinate omitted from a purportedly complete carrier is physically consequential only when some admissible continuation distinguishes the omission; a retained coordinate with no such witness is quotient-null.
\end{principle}

\begin{principle}[Closed-world completeness]\label{pr:ts-closed-world}
Every enforceable distinction of a closed interface belongs to its complete physical ledger.  Conversely, a nonzero element of the reduced physical representation must be exposed by at least one admitted comparison or record sector.  Hidden consequential distinctions and physically empty represented directions are both forbidden in the reduced closed-world description.
\end{principle}

\begin{principle}[A2 no-excess realization]\label{pr:ts-a2}
For fixed enforceable distinction content, the realized carrier is a minimum-cost admissible realization.  A carrier may not retain positive-cost capacity that contributes no additional enforceable distinction.  This no-excess clause compares nested distinction content; it does not choose among physically distinct outcomes.
\end{principle}

\begin{definition}[Held continuation profile]\label{def:ts-held-profile}
A held continuation profile is a contextual equivalence class
\[
H=[\mathcal P]_{\rm ctx}
\]
of pre-record physical presentations that agree in every admissible future continuation context.  Particular domains, groups, coordinates, or amplitudes are presentations of $H$, not the ontology of $H$ itself.
\end{definition}

\begin{definition}[Record quotient and held-to-record transition]\label{def:ts-record-transition}
The invariant record quotient forgets continuation data that no committed readout can retain.  An outcome-bearing record transition is a distinct map from a held profile to a committed record sector.  Thus the held carrier, the invariant record quotient, and the outcome-selection instrument are not identified.
\end{definition}

\begin{audit}[Typing status]
The three-way distinction above is a declared structural typing, reproduced at
the same reading grade as its upstream source.  It is not promoted here to a
primitive-level theorem that physically derives the held carrier, quotient, or
outcome instrument.  Equality of concrete presentations is never substituted
for canonical quotient or isomorphism.
\end{audit}

\begin{definition}[Operational recoverability and closure]\label{def:ts-operational-closure}
For operational realizations $a,b$ on $U$, write $a\preceq_U b$ when every tested distinction carried by $a$ is recoverable from $b$ by admissible deterministic lossless postprocessing.  Mutual recoverability defines classes $[a]_U$.  For a family $S$, $\operatorname{cl}_U(S)$ is the set of classes recoverable from a joint realization of $S$.
\end{definition}

\begin{definition}[Joint-realizability package]
\label{def:ts-joint-realizability}
Let $\mathcal A_U$ be the admitted operational classes and let
$\mathfrak J_U\subseteq\mathcal P_{\rm fin}(\mathcal A_U)$ be the downward-closed
family of jointly realizable finite class sets.  For each
$S\in\mathfrak J_U$, a no-excess minimum joint realization exists and has cost
\[
  \kappa_U(S):=\min_{r\in\mathcal R_U(S)}\Cost_U(r).
\]
All such minima have the same deterministic-lossless recoverability closure,
which defines $\operatorname{cl}_U(S)$.
\end{definition}

\begin{assumption}[Finite joint extension / amalgamation]
\label{ass:ts-finite-joint-extension}
If $S\in\mathfrak J_U$ and
$\operatorname{cl}_U(S)\neq\mathcal A_U$, then there exists
$[a]_U\notin\operatorname{cl}_U(S)$ for which
$S\cup\{[a]_U\}\in\mathfrak J_U$.
\end{assumption}

\begin{definition}[Positive marginal extension floor]
\label{def:ts-positive-extension-floor}
For every jointly realizable new-class extension,
\[
  \kappa_U(S\cup\{[a]_U\})-\kappa_U(S)\ge\floor>0.
\]
This is the typed A2 no-excess plus positive tested-distinction-floor statement
used below.
\end{definition}

\begin{lemma}[A2 distinction-increment lemma --- typed]
\label{lem:ts-a2-increment}
If $S,S\cup\{[a]_U\}\in\mathfrak J_U$ and
$[a]_U\notin\operatorname{cl}_U(S)$, then
\[
  \kappa_U(S\cup\{[a]_U\})\ge\kappa_U(S)+\floor.
\]
\end{lemma}
\begin{proof}
Immediate from Definition~\ref{def:ts-positive-extension-floor}.
\end{proof}

\begin{theorem}[Finite Operational Basis Theorem --- conditional general form]
\label{thm:ts-finite-basis}
Let $U$ satisfy finite capacity, the joint-realizability package of
Definition~\ref{def:ts-joint-realizability}, finite joint extension
(Assumption~\ref{ass:ts-finite-joint-extension}), the positive marginal
extension floor, and closure under deterministic lossless postprocessing.  Then
there exists $B_U\in\mathfrak J_U$ with
\[
  \operatorname{cl}_U(B_U)=\mathcal A_U,
  \qquad
  |B_U|\le\left\lfloor\frac{C_U}{\floor}\right\rfloor.
\]
A closure-redundant member may be deleted until $B_U$ is inclusion-minimal.
\end{theorem}
\begin{proof}
Starting from $S_0=\varnothing$, use finite joint extension whenever closure is
incomplete.  The typed increment lemma gives
$\kappa_U(S_k)\ge k\floor$, while joint admissibility gives
$\kappa_U(S_k)\le C_U$.  Hence the process stops after at most
$\lfloor C_U/\floor\rfloor$ extensions.  Downward closure of
$\mathfrak J_U$ permits redundant-member deletion.
\end{proof}

\begin{corollary}[Finite compatible minimum joint realization]
\label{cor:ts-finite-realization}
For a complete basis $B_U=\{[a_1]_U,\ldots,[a_N]_U\}$, minimum attainment gives
one no-excess joint carrier $r_{B_U}$ and deterministic lossless projections
$\pi_i(r_{B_U})\in[a_i]_U$.  The projected family is finite and compatible
because all members descend from the same joint carrier.  Independently chosen
classwise minima are not asserted to be compatible.
\end{corollary}

\begin{audit}[Finite atom-cover Engine instantiation]
\label{audit:ts-atom-cover-engine-scope}
At APF-Engine v24.3.427 (commit \texttt{7ab898e5ad48aefa620e07e928771727236a8b20}) or its reviewed integration successor, the three checks
\begin{center}
\texttt{T\_finite\_operational\_basis\_scope\_contract},\quad
\texttt{T\_finite\_operational\_basis},\quad
\texttt{T\_finite\_minimal\_joint\_realization\_atom\_cover}
\end{center}
certify the exact finite atom-cover instantiation: finite admitted atoms,
subset-coded classes, union joint carrier and closure, exact no-excess atom
cost, and one compatible minimum joint carrier.  They do not derive finite
joint extension for an arbitrary physical operational closure.
\end{audit}

\begin{warning}[Scope]\label{warn:ts-basis-scope}
Finite operational generation does not imply finitely many terminal values, a finite state space, a manifold, or finite smooth generation.  Precision-refinement channels and differential presentation are separate downstream questions.
\end{warning}

\section{Standard mathematical imports}\label{sec:ts-standard-imports}

\begin{theorem}[Initial-topology separation import]\label{thm:ts-initial-topology}
A family of real-valued functions separating points generates a Hausdorff initial topology.  If a countable subfamily generates that topology and rational intervals are used as a base in each codomain, the resulting topology is second countable.
\end{theorem}

\begin{theorem}[Sikorski differential-space import]\label{thm:ts-sikorski}
Let $P$ be a topological space and $\mathscr C$ a family of real-valued functions closed under localization and smooth superposition: for $f_1,\ldots,f_m\in\mathscr C$ and $\psi\in C^\infty(\mathbb R^m)$, $\psi(f_1,\ldots,f_m)\in\mathscr C$.  Then $(P,\mathscr C)$ is a differential space.  Tangent vectors at $p$ may be defined as derivations of the local algebra.
\end{theorem}

\begin{theorem}[Subcartesian realization import]\label{thm:ts-subcartesian}
If every point of a Hausdorff differential space has a neighbourhood differentially isomorphic to a differential subspace of some $\mathbb R^N$, the space is subcartesian.  A finite map $F=(f_1,\ldots,f_N)$ supplies such a realization when it is a homeomorphism onto its image and every local smooth observable factors through $F$ by a smooth Euclidean function.
\end{theorem}

\begin{theorem}[Complete-family orbit import]\label{thm:ts-orbit-import}
On a subcartesian space, the orbit of a family of vector fields that admits local flows and is complete under pushforward by those flows is an immersed smooth manifold.  Its tangent space is the span of the vector-field values along the orbit.
\end{theorem}

\begin{theorem}[Constant-rank quotient import]\label{thm:ts-constant-rank}
For a smooth bundle map of locally constant rank, the kernel is a smooth subbundle, the quotient by the kernel is a smooth vector bundle, and the induced map from the quotient to the image is a bundle monomorphism.
\end{theorem}

\section{Dependency spine of the conditional package}\label{sec:ts-dag}
The proof DAG used in this supplement is
\[
\begin{aligned}
&\text{finite capacity + joint-realizability package}\\
&\quad+\text{finite joint extension + positive marginal extension floor}\\
&\qquad\Longrightarrow \text{finite operational basis}
\quad\text{(conditional general; exact atom-cover witness)}\\
&\qquad\xrightarrow{\;\text{faithful operational-to-channel gate}\;}
\text{finite separating precision-channel basis}\\
&\qquad\xrightarrow{\;\text{quotient-faithful real limits + topology generation}\;}
\text{Hausdorff, second-countable record topology}\\
&\qquad\xrightarrow{\;\text{finite smooth generation}\;}
\text{subcartesian APF differential space}\\
&\qquad\xrightarrow{\;\text{complete local continuation flows}\;}
\text{smooth continuation orbits}\\
&\qquad\xrightarrow{\;\text{smooth response-factor completeness}\;}
N^{\rm ctx}=\ker D\beta\\
&\qquad\Longrightarrow \text{intrinsic response-orbit quotient gauge}\\
&\qquad\xrightarrow{\;\text{physical substrate anchor}\;}
\text{physical comparison-substrate geometry}.
\end{aligned}
\]
Every arrow carrying a label is a named gate rather than a consequence of the
preceding line alone.  The causal-signature, weak-field no-trace, and nonlinear
Paper~6 translation gates occur strictly downstream.
...
\midrule
Finite operational basis & \Status{Cond./model} & The general theorem is derived only after finite joint extension, minimum attainment, closure invariance, and the positive marginal extension floor are named.  The Engine certifies the exact finite atom-cover instantiation and one compatible minimum joint carrier. \\
Finite physical precision-channel generation & \Status{G} & Requires the faithful operational-to-channel realization gate; the operational basis theorem alone does not supply channel towers, state separation, or real limits. \\
Record topology & \Status{P$\mid$G} & Quotient-faithful real channel limits generate the initial topology; a separating family gives Hausdorffness.  Second countability additionally requires a countable specification grammar and a countable topology-generating subfamily. \\
Differential-space closure & \Status{P/D} & Lossless deterministic postprocessing preserves physical fibres; localization and smooth saturation define the minimal Sikorski differential structure compatible with the admitted readout algebra. \\
Finite-channel sufficiency & \Status{P/G} & Once a complete jointly realizable channel basis exists, additive standing-capacity accounting makes it finite.  Existence is supplied only conditionally by the operational-basis theorem plus the faithful realization gate. \\
Finite differential sufficiency & \Status{G} & After finite state separation, one must still prove finite smooth generation of the continuation-regular sheaf.  This is the subcartesian regularity gate. \\
Smooth continuation orbits & \Status{I/P} & On the finite-differential-presentation locus, the imported subcartesian orbit theorem makes complete-family continuation orbits immersed smooth manifolds. \\
Regular local response quotient & \Status{P$\mid$H} & Constant rank gives local quotient charts.  The derivative-kernel identity further requires smooth two-way factorization between the complete response and a finite local APF generator; equal fibres alone are insufficient. \\
Response-orbit quotient gauge & \Status{P$\mid$H} & After the smooth-factor gate closes, the injective quotient response pulls the process norm back to a positive homogeneous gauge on the response orbit. \\
Physical comparison-substrate anchor & \Status{G} & Requires a site map and a tangent map equal to the quotient differential of that site map, with direct-comparison calibration and stability. \\
Intrinsic response distance & \Status{P} & Integrating the response-orbit gauge gives a coordinate-invariant path distance that dominates endpoint process displacement. \\
Linear Paper~2 specialization & \Status{P} & For $\Phi(\mathcal F,r)=\mathcal F+T_\Pi r$, the geometry is the flat quotient $R_\Pi/\ker T_\Pi$ and reproduces the optimal response gains. \\
Physical-ledger calibration & \Status{P$\mid$H} & Pointwise comparison of ledger rate with the pullback gauge gives bi-Lipschitz comparison; exact equality requires exact calibration. \\
...
Quadratic positive cost metric & \Status{P$\mid$Q} & A parallelogram certificate converts the response gauge into a positive metric.  Weighted $\ell_1$ ledgers remain Finsler. \\
Lorentzian comparison structure & \Status{O/C} & A separate finite-coordination cone and signed two-homogeneous gate is required; no positive pullback metric can create Lorentzian signature. \\
Point-source exterior profile & \Status{C/P} & An assumed isotropic quadratic exterior kernel gives $\chi(r)\sim1/r$; the kernel and calibration remain inputs. \\
Clock-sector redshift recovery & \Status{R} & Calibrated point-mass load, clock response, and the proper-time dictionary recover the Schwarzschild redshift factor conditionally. \\
Trace--volume theorem & \Status{P/C} & For $q_\chi=R_\chi^*q_0$, the first comparison-volume variation is $\operatorname{tr}L$; after clock calibration it is $1-\gamma_C$. \\
No-trace operator reduction & \Status{P/C} & In gauge-fixed linear response the weak-field gate is $\Ptr\mathcal K^{-1}\mathcal J=0$; deriving it from recruitment remains open. \\
Conditional weak-field exterior & \Status{C/R} & If no trace closes, the first-post-Newtonian Schwarzschild exterior follows in isotropic weak-field form. \\
...
\[
\begin{aligned}
\text{finite executable records}
&\Longrightarrow \text{Hausdorff second-countable record topology} \\
&\Longrightarrow \text{refinement-complete readout sheaf} \\
&\Longrightarrow \text{Sikorski differential space} \\
&\Longrightarrow \text{finite differential presentation, where certified} \\
&\Longrightarrow \text{smooth continuation orbits} \\
&\Longrightarrow T S/N^{\rm ctx}\hookrightarrow T P^{\rm red} \\
&\Longrightarrow \text{pullback gauge and calibrated physical cost} \\
&\Longrightarrow \text{positive cost metric, if quadratic} \\
&\Longrightarrow \text{signed causal geometry, if independently closed} \\
&\Longrightarrow \delta\log\sqrt{|\det q|}=\operatorname{tr}L=1-\gamma_C.
\end{aligned}
\]
\end{minipage}}
\end{center}
...
\begin{enumerate}
\item construct a complete regular response model for comparison continuations;
\item calibrate its tangent gauge to the physical interaction ledger;
\item construct the comparison-response anchor and the independent causal cone;
\item derive the loaded-equilibrium Hessian $\mathcal K$ and source $\mathcal J$;
\item prove the no-trace condition $\Ptr\mathcal K^{-1}\mathcal J=0$.
\end{enumerate}
The first two now have precise finite theorems.  The third and fifth are the new APF-specific targets.  Connections and curvature are deferred until regularity, smoothness, strong convexity or quadraticity, and the causal signature gate are all established.
...
positive-dimensional smooth continuation orbit.  To obtain that orbit, the
physical construction must either pass to an asymptotic quotient that changes
the relevant topology or add independently certified non-profinite smooth
structure.  The statement does not exclude real-valued faithful embeddings of
Cantor-type tower spaces.
\end{corollary}
...
real constants are not admitted as single primitive tokens unless an explicit
countable code is supplied.
\end{assumption}
...
\begin{proof}
The set of finite words over a countable alphabet is countable, and each
specified channel is determined by one such word.
\end{proof}

\begin{gate}[Countable topology generation]
\label{gate:countable-topology-generation}
The finitely specified quotient-faithful real channel limits contain a
countable subfamily whose initial topology equals the record-limit topology
defined by the complete ledger-certified channel family.
\end{gate}
...
Gate~\ref{gate:countable-topology-generation}, the closed-interface
record-limit topology is second countable.
\end{corollary}

\begin{proof}
Inverse images of rational open intervals under the countable
topology-generating subfamily form a countable subbasis; finite intersections
form a countable base.
\end{proof}
...
\begin{gate}[Faithful operational-to-precision-channel realization]
\label{gate:operational-to-precision-channel}
A closed interface $U$ closes the operational-to-channel bridge when there is a
map
\[
  \mathfrak R_U:\mathcal A_U\longrightarrow\mathsf{Ch}_U
\]
from operational classes to physical precision-channel towers satisfying:
\begin{enumerate}[label=(\roman*)]
\item \emph{joint preservation:} $S\in\mathfrak J_U$ implies
$\mathfrak R_U(S)$ is jointly realizable;
\item \emph{closure preservation and reflection:}
\[
  [a]_U\in\operatorname{cl}_U(S)
  \iff
  \mathfrak R_U([a]_U)
  \text{ is deterministically recoverable from }\mathfrak R_U(S);
\]
\item \emph{state completeness:} the full image family separates reduced
physical states; and
\item \emph{faithful real realization:} each complete channel tower has a real
limit with the same complete fibres.
\end{enumerate}
\end{gate}

\begin{theorem}[Conditional finite precision-channel basis]
\label{thm:conditional-finite-channel-basis}
If Theorem~\ref{thm:ts-finite-basis} and
Gate~\ref{gate:operational-to-precision-channel} hold, then the image of a
finite complete operational basis is a finite jointly realizable separating
precision-channel family.  Deleting channel-redundant members yields a complete
channel basis with cardinality at most
$\lfloor C_U/\floor\rfloor$.
\end{theorem}
\begin{proof}
Joint preservation gives simultaneous realizability.  Closure preservation and
state completeness give separation.  Finiteness and the capacity bound are
inherited from the operational basis.  Redundant deletion preserves joint
realizability because the joint domain is downward closed.
\end{proof}

\begin{audit}[Bridge disposition]
The theorem above is a proved implication.  The physical realization map
$\mathfrak R_U$ remains a named gate.  Therefore the operational basis theorem
does not silently close finite physical channel generation.
\end{audit}
...
\begin{audit}[Open-item ledger before regular response geometry]
No topological or smooth conclusion is promoted past its antecedent.  The
remaining substantive certificates are:
\begin{enumerate}[label=(\roman*)]
\item faithful realization of the finite operational basis as a jointly
realizable, state-separating precision-channel family;
\item a countable specification grammar and countable topology-generating
subfamily whenever second countability is claimed;
\item finite smooth generation of the continuation-regular readout sheaf;
\item a locally flowable continuation family complete in the orbit-theorem
sense; and
\item smooth two-way response-factor completeness before the derivative-kernel
identity is used.
\end{enumerate}
A physical comparison-substrate anchor is a further downstream gate after
intrinsic response-orbit geometry has been obtained.
\end{audit}
...
and topology-generation conditions of
Assumption~\ref{ass:countable-channel-grammar} and
Gate~\ref{gate:countable-topology-generation}; a complete finite channel basis;
finite smooth generation; and a locally flowable continuation family complete
in the orbit-theorem sense.  Then:
\begin{enumerate}[label=(\roman*)]
...
\item on $U\cap P^{\rm fdiff}$ the differential space is locally subcartesian;
...
Gate~\ref{gate:ts-smooth-response-factor-completeness},
\[
\ker D\beta_x
=N_x^{\rm ctx}
=\bigcap_{a\in\mathcal A_x}\ker\ell_{a,x},
...
is injective.
\end{enumerate}
Thus ordinary smooth response geometry is the regular orbit locus of the APF
record differential space, not an upstream manifold postulate.
\end{theorem}
...
The differential-space architecture is noncircular, but the following
obligations remain deliberately named:
\begin{enumerate}[label=(\roman*)]
\item faithful real channel realization and state separation;
\item a countable topology-generating channel subfamily when second countability
is claimed;
\item a complete channel basis, obtained here only after the
operational-to-channel realization gate closes;
\item finite smooth generation and the homeomorphism/local-factorization clauses
needed for a subcartesian presentation;
\item a local flow family complete in the orbit-theorem sense; and
\item smooth response-factor completeness before set-level fibre equivalence is
promoted to a derivative-kernel identity.
\end{enumerate}
...
\label{audit:ts-fibre-derivative-counterexample}
On $\mathbb R$, the maps $L(t)=t$ and $\beta(t)=t^3$ have identical fibres, but
$dL_0\neq0$ while $D\beta_0=0$.  Their set-theoretic factor is not smooth at the
origin.  Closed-world fibre completeness is therefore insufficient for the
first-order kernel identity.
\end{audit}

\begin{gate}[Smooth response-factor completeness]
\label{gate:ts-smooth-response-factor-completeness}
On a neighbourhood $V\subseteq S$ of $x$, there is a finite APF readout
generator
\[
  F=(f_1,\ldots,f_N):V\to\mathbb R^N
\]
whose components belong to the admitted readout algebra and generate the local
Sikorski structure, together with smooth maps on the relevant images such that
\[
  F=\Psi\circ\beta,
  \qquad
  \beta=\Theta\circ F.
\]
Equivalently, the complete reduced response and a finite local APF generator
are smoothly equivalent at first order on the declared regular branch.
\end{gate}

\begin{theorem}[Regular-orbit closed-world kernel theorem --- corrected]
\label{thm:regular-stratum-closed-world-kernel}
On a smooth geometric branch satisfying closed-world germ completeness and
Gate~\ref{gate:ts-smooth-response-factor-completeness},
\[
  \boxed{
  \ker D\beta_x
...
function of $F$, so $\ker DF_x$ is contained in every admitted readout kernel.
Conversely the components $f_i$ are themselves admitted readouts, so a vector
annihilated by every admitted readout differential lies in $\ker DF_x$.
\end{proof}
...
\[
  A_x:T_xM_{\rm reg}\longrightarrow T_{s(x)}^{\rm op}S
\]
such that, writing
$q_y:T_yS\to T_y^{\rm op}S=T_yS/N_y^{\rm ctx}$ for the quotient map:
\begin{enumerate}[label=(\roman*)]
\item $\ker A_x$ is exactly the physical comparison-null subspace;
\item the map has declared process/ledger provenance and is invariant under
response-coordinate gauge;
\item the kinematic anchor is the quotient differential of the site map,
\[
  A_x=q_{s(x)}\circ Ds_x;
\]
\item the direct comparison germ is calibrated by
$c_x^{\rm dir}(u)=c_{s(x)}^{\rm op}(A_xu)$; and
\item the calibration is stable on the declared regular branch.
\end{enumerate}
\end{gate}

\begin{theorem}[Conditional physical substrate pullback]
\label{thm:anchored-cost-geometry}
Write $L_{\rm orb}[\delta]:=\int c^{\rm op}_{\delta(t)}([\dot\delta(t)])\,dt$
for the response-orbit length (\Cref{thm:canonical-operational-anchor}) of a
piecewise $C^1$ curve $\delta$ in $S$.  If
\Cref{gate:physical-substrate-anchor} holds, define
...
\]
Thus response-orbit geometry becomes physical comparison-substrate geometry
only after the anchor gate closes.
\end{theorem}

\begin{proof}
By Gate~\ref{gate:physical-substrate-anchor}(iii),
\[
A_{\gamma(t)}\dot\gamma(t)
=q_{s(\gamma(t))}\!\left(Ds_{\gamma(t)}\dot\gamma(t)\right)
=[(s\circ\gamma)'(t)].
\]
Substitution into the definition of $c^{\rm sub}$ gives equality of the two
integrands, hence equality of the lengths.  Clauses (i), (ii), (iv), and (v)
supply the physical null-space, provenance, calibration, and regularity
conditions.
\end{proof}
...
Regular response quotient & \Status{P$\mid$H} & Proved locally under constant rank; the physical comparison response model and its rank stratification remain to be constructed. \\
Pullback process gauge and response distance & \Status{P} & Proved on every regular quotient; smooth/strongly convex Finsler structure requires extra norm regularity. \\
Linear Paper~2 specialization & \Status{P} & Proved as the flat quotient branch with optimal upper and lower gains. \\
Physical-ledger calibration & \Status{C/P} & The comparison theorem is proved; the actual ledger-rate bounds and response-class completeness are physical inputs. \\
Residual energy--distance relation & \Status{P$\mid$SRR} & Proved locally when the residual Hessian is positive definite. \\
Comparison-response anchor & \Status{O/C} & Construct $A_x$ and show the anchored germ agrees with direct comparison cost. \\
Quadratic positive cost metric & \Status{P$\mid$Q} & Proved from the parallelogram certificate; whether the substrate passes the certificate is open. \\
...
Dimension lock-in & \Status{O} & Requires an anchor-aware mode-count theorem, not merely a register-count argument. \\
...
No-trace operator $\Ptr\mathcal K^{-1}\mathcal J=0$ & \Status{O/C} & Decisive local theorem; source-selection and constrained fixed-volume are the two principal routes. \\
...
Full Einstein equations & \Status{I/C} & Imported from Paper~6 under H1--H3/A9/Lovelock; residual finite-continuability translation remains open. \\
\bottomrule
\end{longtable}
...
\midrule
G9.1 & Constant-rank response gives a local quotient chart. & \Status{P$\mid$H} & \Cref{prop:constant-rank-response-quotient}. \\
G9.2 & The process norm pulls back to a quotient tangent gauge. & \Status{P} & \Cref{thm:pullback-gauge}. \\
G9.3 & Intrinsic response length dominates endpoint process displacement. & \Status{P} & \Cref{thm:endpoint-process-bound}. \\
G9.4 & The finite linear chart is flat and recovers Paper~2 gains. & \Status{P} & \Cref{thm:linear-flat-response-geometry}. \\
G9.5 & Calibrated physical path cost is bi-Lipschitz to response distance. & \Status{P$\mid$H} & \Cref{thm:ledger-calibration}. \\
G9.6 & Smooth residual energy is one half squared local distance. & \Status{P$\mid$SRR} & \Cref{thm:residual-energy-distance}. \\
G9.7 & Response-orbit geometry pulls back to the physical substrate only after the anchor gate closes. & \Status{C} & \Cref{gate:physical-substrate-anchor,thm:anchored-cost-geometry}. \\
G9.7a & Operational basis to physical precision-channel realization is a named open bridge. & \Status{O/C} & \Cref{gate:operational-to-precision-channel}. \\
G9.7b & Equal closed-world fibres need smooth response-factor completeness for the derivative-kernel identity. & \Status{C} & \Cref{gate:ts-smooth-response-factor-completeness,thm:regular-stratum-closed-world-kernel}. \\
G9.7c & The response-orbit quotient gauge is intrinsic; a physical site anchor is a separate gate. & \Status{P/O} & \Cref{thm:canonical-operational-anchor,gate:physical-substrate-anchor}. \\
G9.8 & Parallelogram quadraticity gives a positive cost metric. & \Status{P$\mid$Q} & \Cref{prop:positive-cost-metric}. \\
G9.9 & Positive pullback geometry cannot supply Lorentzian signature. & \Status{P} & \Cref{prop:positive-pullback-no-lorentz}. \\
G9.10 & A signed Lorentz--Finsler comparison structure represents the causal cone. & \Status{O/C} & \Cref{gate:signed-comparison}. \\
G9.11 & Isotropic quadratic exterior recruitment gives $\chi\sim1/r$. & \Status{C/P} & \Cref{prop:point-source-profile}. \\
G9.12 & Calibrated clock response recovers redshift. & \Status{R} & Response law and proper-time dictionary. \\
G9.13 & Generator trace equals comparison-volume variation. & \Status{P/C} & \Cref{thm:trace-volume}. \\
G9.14 & Linear loaded equilibrium is $h_\chi=\chi\mathcal K^{-1}\mathcal J+O(\chi^2)$. & \Status{P$\mid$H} & \Cref{thm:no-trace-operator}. \\
G9.15 & The no-trace gate is $\Ptr\mathcal K^{-1}\mathcal J=0$. & \Status{O/C} & Source-selection or constrained-volume theorem. \\
G9.16 & No trace gives $\gamma_C=1$ and the first-PN exterior. & \Status{C/R} & \Cref{cor:ppn-volume-mode,thm:conditional-weak-field}. \\
G9.17 & The represented nonlinear closure is Einstein gravity. & \Status{I/C} & Paper~6 H1--H3/A9/Lovelock import. \\
\bottomrule
\end{longtable}
...
\item the comparison-response anchor and the explicit separation of positive cost from Lorentzian comparison geometry;
...
\begin{quote}
This supplement is dependency-closed as a conditional proof package when every
open gate above appears as a typed node in the theorem ledger.  Dependency
closure does not mean physical realization closure.  In particular, finite
joint extension on arbitrary operational closures, the faithful
operational-to-channel realization, finite smooth generation, smooth
response-factor completeness, the physical substrate anchor, quadraticity,
signed causal geometry, the physical no-trace source law, and the nonlinear
Paper~6 endpoint remain conditional, open, or external as explicitly marked.
\end{quote}
...
The same architecture resolves the homogeneity ambiguity.  One-homogeneous path ledgers generate length.  Smooth residual penalties generate local quadratic energy, and their correct geometric form is one half squared distance.  Neither branch should be renamed into the other.  The comparison-response anchor is now placed on a derived record differential space.  Closed-world precision channels generate the topology; lossless postprocessing preserves the physical quotient; smooth saturation supplies the Sikorski functional calculus; finite capacity gives finite-channel sufficiency; finite differential sufficiency identifies the subcartesian locus; and locally complete admissible continuation derivations generate immersed smooth manifold orbits.  The anchor therefore acts on emergent smooth continuation branches rather than on an assumed substrate manifold.  Under quadraticity it yields a positive cost metric, but a separate causal-cone and signed two-homogeneous gate is required for Lorentzian comparison geometry.  This blocks the misleading shortcut $g=T^*T$ as a derivation of spacetime signature while preserving the valid pullback construction for positive cost.
...
Paper~6 remains the represented smooth source of record for the Einstein closure.  Paper~9 supplies the intrinsic response geometry underneath it: quotient, pullback gauge, path distance, ledger calibration, anchor, causal signature gate, recruitment deformation, and trace--volume response.  That is the coherent role of the paper and the correct home of the interaction-geometry program.
